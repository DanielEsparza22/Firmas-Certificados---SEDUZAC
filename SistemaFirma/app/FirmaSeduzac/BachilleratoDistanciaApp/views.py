...
from .forms import RegistrosBachilleratoDForm
...
def obtener_registros_insertar(cursor, clave, fecha_cert, fecha_certificacion, semestre):
    query = (
        ...
        "WHERE e.clave_ct = %s "
        "AND a.semestre = %s "
        "AND a.promedio >= 6 AND a.periodo_s4 IS NOT NULL "
...
def get_curp_bachiller_distancia(cursor, clavecct):
    query = (
        ...
        "AND ac.promedio >= 6 AND a.periodo_s4 IS NOT NULL "
        "AND a.cve_alumno NOT IN (SELECT f.cve_alumno FROM calificaciones f WHERE f.cve_alumno=a.cve_alumno AND (f.calificacion<6 OR f.ban_acreditada='X')) "
        "AND a.alumno_estatus IN ('R', 'N', 'P', 'A');"
    )
...
def obtener_valores_cadena(cursor, curp):
    ...
    cadena = f"||1.0|3|SAFC710514MDFLLR06|Secretaria de Educación|SECRETARÍA DE EDUCACIÓN DEL ESTADO DE ZACATECAS|Secretaría de Educación del Estado de Zacatecas|{clavecct}|056|32|{curp_alumno}|{nombre}|{ap_paterno}|{ap_materno}|3|{promedio}|{fecha_final}||"
    # print(cadena)
        ...
        with connections['mariadb'].cursor() as cursor:
            if(form_registros.is_valid()):
                ...
                if('obtener_registros' in request.POST):
                    ...
                    if(clavecct):
                        curp = get_curp_bachiller_distancia(cursor,clavecct)
                    ...
                    if(clavecct):
                        curp = get_curp_bachiller_distancia(cursor,clavecct)
                        foliar_certificado_prepas(cursor, curp)
