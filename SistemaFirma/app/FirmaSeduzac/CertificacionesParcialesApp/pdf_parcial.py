...
def pdf_parciales(curp):
    with connections['mariadb'].cursor() as cursor:
        query = ("SELECT id_hist_estudio, curp, prim_apellido, seg_apellido, nombre, nombre_ct, clavecct, "
                 "bachillerato, fecha_cert_texto, folio_sep, sello_seduzac, autoridad_educativa, certificado_autoridad_educativa, fecha_firma FROM alumno_cert_parcial WHERE curp = %s;")
        ...
        query2 = ("SELECT a.id_hist_estudio, a.curp, b.cve_materia, m.nombre_materia, b.promedio "
                  "FROM alumno_cert_parcial a JOIN boleta b ON a.id_hist_estudio = b.cve_alumno JOIN "
        ...
        query_semestres = {
            1: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 1 AND b.periodo_s1 IS NOT NULL;",
            2: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 2 AND b.periodo_s2 IS NOT NULL;",
            3: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 3 AND b.periodo_s3 IS NOT NULL;",
            4: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 4 AND b.periodo_s4 IS NOT NULL;",
            5: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 5 AND b.periodo_s5 IS NOT NULL;",
            6: "SELECT m.nombre_materia, b.calificacion FROM boleta b JOIN materias m ON b.cve_materia = m.cve_materia WHERE b.curp = %s AND b.semestre = 6 AND b.periodo_s6 IS NOT NULL;"
        }

        # Diccionario para almacenar los resultados por semestre
        materias_por_semestre = {}

        # Ejecutar las consultas para cada semestre
        ...
        # Variables para saber si los semestres han sido cursados
        ps1 = periodo_s1[0] if periodo_s1 else None
        ps2 = periodo_s2[0] if periodo_s2 else None
        ps3 = periodo_s3[0] if periodo_s3 else None
        ps4 = periodo_s4[0] if periodo_s4 else None
        ps5 = periodo_s5[0] if periodo_s5 else None
        ps6 = periodo_s6[0] if periodo_s6 else None 
        ...
        total_materias = sum(len(materias_por_semestre[semestre]) for semestre in range(1, 7) if (semestre == 1 and ps1) or (semestre == 2 and ps2) or (semestre == 3 and ps3) or (semestre == 4 and ps4) or (semestre == 5 and ps5) or (semestre == 6 and ps6))

        # Variables para nombres de mtaerias y promedios
        nombres_materias = []
        promedios = []

        # Iterar sobre los resultadoos y almacenar nombres de materia y promedios
    ...
    try:
        # Dibuja la imagen de fondo abarcando todo el PDF
    ...
    titulo = f"Certificado Parcial - {curp}"
    p.setTitle(titulo)
    ...
    if(total_materias <= 51):
        p.setFont("Helvetica", 9)
        texto_p2 = (f"La presente certificación ampara {total_materias} de 51 asignaturas que integran el Plan de Estudios respectivo.")
        p.drawString(x_texto, y_texto - 25, texto_p2)
    elif(total_materias > 51):
        p.setFont("Helvetica", 9)
        texto_p2 = (f"La presente certificación ampara {total_materias} de {total_materias} asignaturas que integran el Plan de Estudios respectivo.")
        p.drawString(x_texto, y_texto - 25, texto_p2)

    p.setFont("Helvetica", 9)
    texto_p3 = (f"El presente se expide en GUADALUPE, ZACATECAS, a los {fecha_texto}.")
    p.drawString(x_texto, y_texto-40, texto_p3)
    ...
    data = [['ASIGNATURAS', 'CALIF.FINAL ', 'ASIGNATURAS', 'CALIF.FINAL']]

    def agregar_semestre(titulo, semestre, titulo2=None, semestre2=None):
        if (semestre in materias_por_semestre and materias_por_semestre[semestre]) or (semestre2 in materias_por_semestre and materias_por_semestre[semestre2]):
            data.append([titulo, '', titulo2 if titulo2 else '', ''])
            max_length = max(len(materias_por_semestre.get(semestre, [])), len(materias_por_semestre.get(semestre2, [])))
            ...
            if(max_length == 0):
                ...
                for i in range(max_length):
                    materia1 = materias_por_semestre[semestre][i] if i < len(materias_por_semestre.get(semestre, [])) else ('', '')
                    materia2 = materias_por_semestre[semestre2][i] if i < len(materias_por_semestre.get(semestre2, [])) else ('', '')
    ...
    if(ps1 and ps2):
        agregar_semestre(Paragraph(f"PRIMER SEMESTRE({ps1})", title_style), 1, Paragraph(f"SEGUNDO SEMESTRE({ps2})", title_style), 2)
    elif(ps1 and not ps2):
        agregar_semestre(Paragraph(f"PRIMER SEMESTRE({ps1})", title_style), 1)

    # Tercer a Sexto Semestre
    if(ps3 and ps4 and ps5 and ps6):
        agregar_semestre(Paragraph(f"TERCER SEMESTRE({ps3})", title_style), 3, Paragraph(f"CUARTO SEMESTRE({ps4})", title_style), 4)
        agregar_semestre(Paragraph(f"QUINTO SEMESTRE({ps5})", title_style), 5, Paragraph(f"SEXTO SEMESTRE({ps6})", title_style), 6)
    elif(ps3 and ps4 and ps5 and not ps6):
        agregar_semestre(Paragraph(f"TERCER SEMESTRE({ps3})", title_style), 3, Paragraph(f"CUARTO SEMESTRE({ps4})", title_style), 4)
        agregar_semestre(Paragraph(f"QUINTO SEMESTRE({ps5})", title_style), 5)
    elif(ps3 and ps4 and not ps5):
        agregar_semestre(Paragraph(f"TERCER SEMESTRE({ps3})", title_style), 3, Paragraph(f"CUARTO SEMESTRE({ps4})", title_style), 4)
    elif(ps3 and not ps4):
        agregar_semestre(Paragraph(f"TERCER SEMESTRE({ps3})", title_style), 3)

    # Relleno hasta llegar a 35 o 37 filas
    if(ps1 and ps2 and ps3 and ps4 and ps5 and ps6):
        while len(data) < 35:
            data.append([" ", " "])
    else:
        while len(data) < 37:
    ...
    # Dibujo el rectangulo para la celda
    p.setLineWidth(0.1)
    ancho_rect = 79  # Ancho del rectangulo
    alto_rect = 21 # Alto del rectangulo
    p.setFont("Helvetica", 9)
    p.rect(x_texto + 18, y_texto -498, ancho_rect, alto_rect)
    # p.drawString(x_texto + 48, y_texto - 439,f'{promedio_total}')

    # Codigo QR
    qr_text = f"{nombre} {prim_apellido} {seg_apellido} Secretaría de Educación del Estado de Zacatecas Certificación de Estudios https://www.seduzac.gob.mx/consultaCalificacion.html"  # Texto para generar el QR
